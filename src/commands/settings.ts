...
export function makeSettingsCommand(outOpts: () => OutputOptions): Command {
  const settings = new Command("settings").description("Manage user settings");
  ...
  settings
    .command("update")
    .description("Update user settings (reads JSON from stdin or --file)")
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        const client = await HealthClient.create();
        const result = await client.updateSettings(body, opts.updateMask as string | undefined);
        print(result, o);
      } catch (err) {
        handleError(err, o);
      }
